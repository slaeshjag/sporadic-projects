...
void textbox_init(int w, int h, int x, int y, int pad_x, int pad_y, int pad_x2, int pad_y2) {
	...
	tb->text = d_text_surface_color_new(world.config.font, 2048, tb->surface_w, x + pad_x, y + pad_y);
	tb->rows = (h - pad_y - pad_y2) / d_font_glyph_hs(world.config.font);
	tb->tc = d_tilemap_new(0xFF, world.config.ts_sys, 0xFFF, w / world.config.tile_w, h / world.config.tile_h);
	...
	for (; tb->dt > tb->ms_per_char && tb->message[tb->char_pos]; tb->dt -= tb->ms_per_char) {
		...
		if (tb->message[tb->char_pos]) {
			if (tb->message[tb->char_pos] == ' ') {
				...
				if (d_text_surface_pos(tb->text) + d_font_word_w(world.config.font, 
...
void textbox_add_message(const char *message, const char *question, int face, int pingback) {
	...
	d_tilemap_camera_move(tb->qt, -(d_platform_get().screen_w - wt * world.config.tile_w), -(d_platform_get().screen_h - (ht + tb->tc->h) * world.config.tile_h));
